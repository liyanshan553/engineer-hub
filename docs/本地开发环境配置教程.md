...
如果执行 `git checkout` 时出现下面这类报错：

- `pathspec 'origin' did not match any file(s) known to git`
- `pathspec '<branch-name>' did not match any file(s) known to git`

通常表示本地仓库还没有配置对应远程，或者目标分支在本地/远程都不存在。可以按下面顺序排查：

1. 查看当前远程：`git remote -v`
2. 若没有远程，先添加：`git remote add origin <repo-url>`
3. 拉取远程分支信息：`git fetch origin`
4. 查看可用分支：`git branch -a`
5. 切换到目标分支：`git checkout <branch-name>`（或 `git checkout -b <branch-name> origin/<branch-name>`）
